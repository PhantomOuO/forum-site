...
//用戶選擇看板文章-資料庫搜尋 (預設lang_type:1 HTML)
//預設顯示程式看板
$num_lang = 1;
//變更看板時更新看板搜尋
if (isset($_GET["lang_type"])) {
  $num_lang = $_GET["lang_type"] ;
}
          ...
          <div class="p-3 h-100  rounded-2 bg-light">
            ...
            <nav class="nav flex-column">
              ...
              while ($row_result_lang = $result_lang->fetch_assoc()) {
                echo "<a class='nav-link' href='test.php?lang_type=" . $row_result_lang["language_id"] . "'>" . $row_result_lang["name"] . "</a>";
              }
              $db_link->close();
              ?>
            </nav>
